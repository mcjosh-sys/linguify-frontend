...
@Component({
  ...
  template: `
    <ng-content />
    <button brnSheetClose hlm #sheetClose>
      <span class="sr-only">Close</span>
      <hlm-icon class="flex w-4 h-4 text-primary" size="100%" name="lucideX" />
    </button>
  `,
})
export class HlmSheetContentComponent {
  @ViewChild('sheetClose') closebtn!: ElementRef;
  ...
  constructor(private eventService: EventService) {
    ...
    this.eventService.listen('close-sheet', () => {
      if (this.state() === 'open') {
