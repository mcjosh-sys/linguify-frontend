...
@Component({
	...
	template: `
		<hlm-icon size="sm" [name]="iconName()" [class.text-primary-foreground]="_checked()" />
	`,
})
export class HlmCheckboxCheckIconComponent {
	...
	protected _computedClass = computed(() =>
		hlm(
			'h-4 w-4 leading-none group-data-[state=unchecked]:opacity-0 font-bold flex items-center justify-center',
			this._checked() === 'indeterminate' ? 'opacity-50' : '',
			!this._checked() && 'opacity-0',
			this.userClass(),
		),
	);
}
